...
class TSPEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mask = False
        self.spec = SimpleNamespace(reward_threshold=500)
        self.render_ready = False
        utils.assign_env_config(self, kwargs)

        self.nodes = np.arange(self.N)
        self.step_limit = 3*self.N
    ...
    def plot_network(self, offset=(0.02, 0.02)):
        ...
        for n, c in self.node_dict.items():
            for k in c:
                line = np.vstack([self.coords[:, n], self.coords[:, k]])
            ...
            # else:
            self.ax.annotate(r"$N_{:d}$".format(n), xy=(self.coords[:,n]+offset), zorder=2)
            ...
            if self.current_node == n:
                clr = 'blue'
            # print(clr)
            self.ax.scatter([self.coords[0,n]], [self.coords[1,n]], color=clr, s=400)
